...
contract OperationTest is Setup {
    ...
    function test_healthcheck(uint256 _amount) public {
        ...
        Comet(comet).withdraw(borrowToken, strategy.balanceOfDebt() / 2);
        ERC20(borrowToken).safeTransfer(
            management,
            ERC20(borrowToken).balanceOf(address(depositor))
        );
        vm.stopPrank();
    ...
    function test_availableWithdrawLimit(uint256 _amount) public {
        ...
        uint256 baseBalance = ERC20(borrowToken).balanceOf(comet);
        uint256 debt = strategy.balanceOfDebt();

        // Simulate only enough liquidity to repay half our debt
        vm.prank(comet);
        ERC20(borrowToken).safeTransfer(management, baseBalance - (debt / 2));
